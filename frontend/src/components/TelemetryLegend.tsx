...
export function TelemetryLegend({ lanes, cursorT }: Props) {
  const rows = lanes.flatMap((lane) => {
    ...
    // (e.g. Pédales) can have a comparison for every member, not just the first.
    if (lane.compare) {
      ...
      lane.series.valueColumns.forEach((col, i) => {
        ...
        primary.push({
          ...
          dashed: true,
          label: `${lane.columnStyles[i].label} (comparé)`,
          unit: lane.series.unit,
  ...
  return (
    <div className="telemetry-legend">
      {rows.length === 0 && <div className="legend-empty">Sélectionne des canaux ci-contre</div>}
      {rows.map((r) => (
